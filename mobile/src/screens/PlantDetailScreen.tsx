...
interface ActiveCycleData {
  cycle: { id: string; plantId: string; startedAt: string; finishedAt: string | null } | null;
  mesasDone: number;
  totalMesas: number;
  percentage: number;
  sessionCount: number;
...
export default function PlantDetailScreen({ navigation }: any) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error?.response?.data?.message || error.message || "Error al cargar datos",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error?.response?.data?.message || error.message || "No se pudo iniciar la sesión",
      );
    }
  };
          ...
          <View style={s.card}>
            ...
            <View style={s.progressRow}>
              <Text style={s.progressText}>
                {activeCycle.mesasDone}/{activeCycle.totalMesas} ({activeCycle.percentage}%)
              </Text>
            </View>
            <View style={s.progressBar}>
              <View
                style={[s.progressFill, { width: `${activeCycle.percentage}%` }]}
              />
            </View>
            <Text style={s.cardMeta}>
              Sesiones: {activeCycle.sessionCount} · Técnicos: {activeCycle.technicianCount}
            </Text>
            ...
            {sessions
              ...
              .map((ses) => (
                ...
                >
                  <Text style={s.sessionDate}>{getSessionDate(ses.startedAt, ses.finishedAt)}</Text>
        ...
        {finishedCycles.length > 0 && (
          <View style={s.card}>
            ...
            {finishedCycles.map((cycle) => (
              ...
              >
                <View>
                  <Text style={s.cycleDate}>{getCycleDate(cycle.startedAt, cycle.finishedAt)}</Text>
                  <Text style={s.cycleMeta}>
                    {cycle.sessionCount} sesiones · {cycle.technicianCount} técnicos
                  </Text>
                </View>
...
const s = StyleSheet.create({
  ...
  resumeBtn: { fontSize: 15, color: "#92400e", fontWeight: "bold", marginTop: 4 },
