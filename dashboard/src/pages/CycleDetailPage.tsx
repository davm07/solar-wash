...
export default function CycleDetailPage() {
  ...
  if (isError || !data) {
    return <p className="text-red-500">Error al cargar el resumen del ciclo</p>;
  }
  ...
  const mesaStatuses = Object.fromEntries(
    data.mesasCycle.map((m) => [m.code, m.status]),
  );
    ...
    <div className="flex flex-col lg:flex-row">
      <div className="max-w-4xl mx-auto mb-3 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div className="bg-olive-50 rounded-xl shadow-sm border p-4 border-olive-200">
            <h2 className="font-semibold mb-2">Duración total</h2>
            <p className="text-2xl font-bold text-olive-700">
              {formatDuration(data.sessionDuration)}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Tiempo de sesión (incluye tiempo inactivo)
            </p>
          </div>

          <div className="bg-olive-50 rounded-xl shadow-sm border p-4 border-olive-200">
            <h2 className="font-semibold mb-2">Tiempo de lavado</h2>
            <p className="text-2xl font-bold text-olive-700">
              {formatDuration(data.washingDuration)}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Tiempo activo lavando mesas
            </p>
          </div>
        </div>
      ...
      {data.svgContent && (
        <div className="flex-1 px-12 flex items-center justify-center lg:max-h-[80vh] mb-3 lg:mb-0">
          <PlantMapViewer mesaStatuses={mesaStatuses} svg={data.svgContent} />
        </div>
      )}
    </div>
  );
